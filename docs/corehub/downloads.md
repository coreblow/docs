...
`corehub install` is the OpenClaw-style user command. It uses `--dry-run` for preview; apply is currently blocked until CoreHub publishes installable CoreBlow plugin archives. `package install` remains the technical command and uses the same verified download path when `--output` is present.
